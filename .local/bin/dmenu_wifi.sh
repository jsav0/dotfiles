...
get_interface() {
    INTFC=$(iwctl device list | iwctlstrip | dmenu -l 10 -p "Select an interface:" | awk '{print $1}') 
}
...
get_ssid() {
    CHOSEN=$(printf "$SCAN_RESULTS" | dmenu -l 10)
...
get_psk() {
    PSK=$(printf 'exit\n' | dmenu -p "Enter WPA/WPA2 Secret Passphrase:" | awk '{print}')
}
...
connect_to_a_network() {
    ...
    [[ "$PSK" == "1" ]] && connect_with_psk
    [[ "$OPEN" == "1" ]] && connect_with_open
}
